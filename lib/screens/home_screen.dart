...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _initWindows() async {
    ...
    // reclaims focus on touch-up. Also prevents duplicate click events
    // that occur because touch generates both pointerup and click.
    ...
    await controller.addScriptToExecuteOnDocumentCreated('''
      (function() {
        ...
        // On touchend, mark the target so we can suppress the duplicate
        // click that the browser fires after touch events.
        document.addEventListener('touchend', function(e) {
          ...
          // Use multiple attempts because HWND focus can be stolen
          // at unpredictable timings.
          var el = e.target;
        ...
        // Suppress duplicate click events generated by touch.
        // Touch fires: touchstart -> touchend -> click.
        // If a click arrives within 400ms on the same target, block it.
        document.addEventListener('click', function(e) {
          var now = Date.now();
          if ((now - lastTouchTime) < 400) {
            var el = e.target;
            if (el === lastTouchTarget || (lastTouchTarget && lastTouchTarget.contains && lastTouchTarget.contains(el))) {
              e.stopPropagation();
              e.preventDefault();
              return false;
            }
